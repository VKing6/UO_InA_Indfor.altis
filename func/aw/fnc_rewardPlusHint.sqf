private ["_veh","_vehName","_vehVarname","_completeText","_reward","_dir"];
	...
	if (_vehVarname isKindOf "Plane" && count smHangarList == 0) exitWith {[-1, {"All reward locations full! No reward given."}] call CBA_fnc_globalExecute};
	if (_vehVarname isKindOf "Helicopter" && count smHeliList == 0) exitWith {[-1, {"All reward locations full! No reward given."}] call CBA_fnc_globalExecute};
