...
{	
	...
	switch (_mission) do
	{
		case "talkToContact":
		{
			...
			*/
			...
			while {!_awayFromBase} do
			{
				...
				if (_position distance (getMarkerPos "respawn") > 500 && (_position distance (getMarkerPos currentAO)) > 600) then
				{
					_awayFromBase = true;
				};
			};
		...
		case "destroyOutpost":
		{
			...
			for "_x" from 1 to (_spawnLevel + 1) do
			{
