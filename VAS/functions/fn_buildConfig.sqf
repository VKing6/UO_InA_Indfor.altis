	...
	case "CfgWeapons":
	{
		...
		for "_i" from 0 to (count _master)-1 do
		{
			...
			if(isClass _class) then
			{
				...
				if(_scope >= 2 && _type in [1,2,4,5,4096,131072] && _picture != "" && _displayName != "") then
				{
						...
						else
					{
						...
						if(_str != "ACRE" && !(_base in VAS_r_weapons) && !(_class in VAS_r_weapons) && !(_displayName in _temp)) then
						{
							_temp set[count _temp,_displayName];
		...
		VAS_pre_items =  ["ACRE_PRC148","ACRE_PRC148_UHF","I_UavTerminal"] + _ret2;
	};
