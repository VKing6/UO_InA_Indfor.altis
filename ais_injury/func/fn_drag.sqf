// by Bon_Inf*
	...
	if (!isNull(_injuredperson getVariable "healer") || {!isNull(_injuredperson getVariable "dragger")}) exitWith {
		[format ["%1 is being assisted.", name _injuredperson],0, 0.035 * safezoneH + safezoneY,5,0.3] spawn BIS_fnc_dynamicText
	};

	if (_injuredperson distance _dragger > 3) exitWith {
		[format ["%1 is too far away to be dragged.", name _injuredperson],0, 0.035 * safezoneH + safezoneY,5,0.3] spawn BIS_fnc_dynamicText
	};

	if (!alive _injuredperson) exitWith {
		[format ["R.I.P. %1", name _injuredperson],0, 0.035 * safezoneH + safezoneY,5,0.3] spawn BIS_fnc_dynamicText
	};

	["lynx_aisDrag",[_injuredperson,_dragger]] call CBA_fnc_globalEvent;
	["lynx_aisSwitchMove",[_injuredperson,"AinjPpneMrunSnonWnonDb"]] call CBA_fnc_globalEvent;
	["lynx_aisPlayAction",[_dragger,"grabDrag"]] call CBA_fnc_globalEvent;
	/*
