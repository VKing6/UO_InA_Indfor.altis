...
if (_dragger call tcb_fnc_checklauncher) exitWith {[format ["You can't carry others while wearing a launcher on your back."],0, 0.035 * safezoneH + safezoneY,5,0.3] spawn BIS_fnc_dynamicText};
...
["lynx_aisPlayAction",[_dragger,"grabCarry"]] call CBA_fnc_globalEvent;
...
if (!alive _injuredperson || {!alive _dragger} || {(_dragger getVariable "tcb_ais_agony")}) then {
	if (alive _injuredperson) then {
		["lynx_aisPlayActionNow",[_injuredperson,"agonyStart"]] call CBA_fnc_globalEvent;
